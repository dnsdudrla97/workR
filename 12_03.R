...
plot(wt, mpg,main='중량-연비 그래프',
     xlab='중량',ylab='연비',
     col='red',pch=19) # pch => 산점도의 점 모양이 달라짐(0-25)
...
target <- mtcars[,vars]
head(target)
pairs(target,main="multi plots")     # 변수가 여러개일 때는 pairs사용
                                     # 두변수간의 관계가 강하면 사선!

# 그룹정보가 있는 두변수의 산점도 -> 상관관계를 알아보기 위해서 사용
iris.2 <- iris[,3:4]
...
tb1 <- data.frame(beers,bal)  # 다변량 일때는 두개 이상의 변수를 비교하기 위해서
                              # data.frame사용
                              # 일변량일때는 vector를 사용해도 됨
tb1
plot(bal~beers,data=tb1)      # 산점도

res <- lm(bal~beers,data=tb1) # 회귀식
res

abline(res, col='red')        # 회귀선

cor(tb1[,1:2])                # 이변량 상관 계수
                              # |0.5|<= :강한 상관 관계, |0.5|>: 상관 관계가 없다.
...
plot(month,late,main='지각생통계',
      type='l',lty=1,lwd=1,              #lty: 선의 모양, lwd: 선의 두께
...
# 0단계: 문제 정의
# 1단계: 분석 대상 데이터셋 준비
#        BostonHousing 데이터섹(mlbench pac.)
